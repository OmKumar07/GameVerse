...
// @route   GET /api/public/users/trending
// @desc    Get trending/popular users
// @access  Public
router.get("/users/trending", async (req, res) => {
  try {
    const { limit = 10 } = req.query;

    // Find users with most activity (simplified - could be enhanced with more complex algorithms)
    const trendingUsers = await User.find({
      isActive: true,
      profilePrivacy: { $ne: "private" },
    })
      .select(
        "username displayName profileImage bio totalGamesPlayed followers"
      )
      .sort({
        totalGamesPlayed: -1,
        loginCount: -1,
    ...
    const formattedUsers = trendingUsers.map((user) => ({
      ...
      stats: {
        totalGamesPlayed:
          user.showGameStats !== false ? user.totalGamesPlayed : undefined,
    ...
    res.status(500).json({
      error: "Trending Users Fetch Failed",
      message: "Unable to fetch trending users",
    });
  }
});
